...
| **`atrium-page-classification`** | Missing central security wrapper & missing PO secret vault | 1. **Delete** `image-scan.yml` and `version-check.yml`.  2. **Add** the central `security.yml` caller. 3. **Add** `secret-scan.yml` (P0 Priority blocker). |
| **`atrium-alto-postprocess`**    | Redundant version-checking pipelines                       | **Delete** `.github/workflows/version-check.yml` (Handled by the central security loop).                                                                   |