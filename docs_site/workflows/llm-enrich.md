...
ATRIUM document record without OCR.
...
| **Compute**        | one or more GPUs for local models; none when a remote provider answers                                                                                                           |
| **Network**        | the AIS CR services, to harvest the vocabularies; the Hugging Face Hub for local models, or the remote provider                                                                  |
| **Code licence**   | MIT                                                                                                                                                                              |
...
| 2 | Prepare the text        | Lines are read from the line tables or from TEITOK; whole documents are turned into Markdown. Born-digital PDF and DOCX files can be converted to the document record here instead of going through OCR. | CSV, TEITOK XML, PDF, DOCX → Markdown, JSON | Converting                  |
...
provider its own terms; both apply to what the model produced. The born-digital converter uses
MIT-licensed libraries, and the source document's own licence applies to its text.
...
Read from `ufal/atrium-llm-enrich` at release **`v0.7.0`**. This table records
